...
export function startPresentation(
  ...
  const goNext = () => {
    const stepResult = steps.next()
    if (stepResult.kind === 'step') return revealStep(stepResult)
    hideEndScreen()
    resetAutoAdvance()
    const cur = deck.getIndices().h
  ...
  const goPrev = () => {
    const stepResult = steps.prev()
    if (stepResult.kind === 'step') return hideStep(stepResult)
  ...
  const hasNext = () => steps.hasNext() || !endScreenShown
  const hasPrev = () => {
  ...
  let inkBuilt = false
  /** Last known pointer position (fraction of the overlay) — used only to
   *  place a term when typing starts via the keyboard shortcut (see the
   *  keydown handler further down) rather than tapping the canvas first. */
  let lastPointerFrac: InkPoint = { x: 0.5, y: 0.5 }
  ...
  const buildInk = () => {
    ...
    inkCanvas.addEventListener('pointerdown', (ev) => {
      ...
      if (inkTool === 'hand') {
        ...
        if (termHit) startDragTerm(termHit.index, p)
        return
      }
  ...
  const setInkEnabled = (on: boolean) => {
    ...
    if (on) buildInk()
    else { cancelTermInput(); setInkTool('pen') }
    inkEnabled = on
    inkStroke = null
  ...
  inkHandBtn.title = t('Hand: Begriffe an eine andere Stelle ziehen')
  ...
  inkEraseBtn.title = t('Radierer (Doppelklick auf einen Begriff löscht ihn ganz)')
  ...
  const updateSpeakerControls = () => {
    ...
    if (count) {
      const maxStep = doc.slides[cur] ? Math.max(0, ...doc.slides[cur].elements.map(stepOf)) : 0
      count.textContent = `${visibleIndex(cur)} / ${visibleTotal}` + (maxStep ? ` · ${steps.step}/${maxStep}` : '')
    }
  ...
  const onKeydown = (ev: KeyboardEvent) => {
    // The floating term input has its own keydown handler (Escape
    ...
    // Typing while the pen is on places a term directly at the last pointer
    // position — no need to switch to the T tool first. Modifier combos
    // (⌘C, etc.) pass through untouched.
    if (inkEnabled && !termInput && ev.key.length === 1 && !ev.ctrlKey && !ev.metaKey && !ev.altKey) {
      ev.preventDefault()
      ev.stopPropagation()
      buildInk()
      placeTermInput(lastPointerFrac, ev.key, null)
      return
    }
  ...
  function showRefPopover(anchor: HTMLElement, text: string) {
    ...
    const closeOnAway = (ev: MouseEvent) => {
      ...
      pop.remove()
      document.removeEventListener('click', closeOnAway, true)
    }
    setTimeout(() => document.addEventListener('click', closeOnAway, true), 0)
  }
  ...
  function renderLongRead(idx: number) {
    ...
    for (const block of lr?.blocks ?? []) {
      ...
      if (block.type === 'references') {
        const ul = document.createElement('ul')
        ul.className = 'bento-longread-references'
  ...
  deck.initialize().then(() => {
    ...
    if (first) {
      steps.enter(doc.slides[effectiveStart]?.elements ?? [], true)
      applyStep(first, doc.slides[effectiveStart], steps.step)
      if (!reduceMotion) {
        runEnterFx(doc.slides[effectiveStart], first, undefined, false, steps.step)
 ...
 * stepped element that simply pops in reads as a glitch, not a reveal.
 * `atStep` (slide entry) leaves elements hidden by the step counter alone —
 * they run their entrance when their own step comes.
 */
