...
// download fallback, the export copies, and window.bento.serialize()) first
// drops the assets nothing refers to. The kernel never learns the slides
// document shape; it is handed a document with fewer bytes in it. See
// pruneUnusedAssets (model.ts) for why, and for the live document being
// untouched.
...
import { pruneUnusedAssets } from './model'
